...
create policy "platform_admins_manage_subscriptions" on public.saas_subscriptions for all to authenticated using (exists (select 1 from public.platform_admins pa where pa.user_id = auth.uid())) with check (exists (select 1 from public.platform_admins pa where pa.user_id = auth.uid()));
...
create policy "platform_admins_manage_saas_payments" on public.saas_payments for all to authenticated using (exists (select 1 from public.platform_admins pa where pa.user_id = auth.uid())) with check (exists (select 1 from public.platform_admins pa where pa.user_id = auth.uid()));
...
create policy "platform_admins_view_all_clubs" on public.clubs for select to authenticated using (exists (select 1 from public.platform_admins pa where pa.user_id = auth.uid()));
